...
function persona(p, titolo, idx, tipo) {
  // Blocco persona (locatore o conduttore)
  ...
  var suf = idx > 0 ? ';' : ',';
  return '<p>' + titolo + ' ' + nome + ', nato/a a ' + luogo + ' il ' + data +
    ', <b>C.F.: </b>' + cf + ', residente in ' + res + ', Via ' + via +
...
function bloccoImmobile(d, allegato) {
  var comune = vb(d.comune || (d.appartamento && d.appartamento.comune), 100);
  var via    = vb(d.via || (d.appartamento && d.appartamento.via), 160);
  var nciv   = vb(d.nCivico || '', 40);
  var piano  = v(d.piano || (d.appartamento && d.appartamento.piano), 60);
  var scala  = v(d.scala || (d.appartamento && d.appartamento.scala), 40);
  var interno= v(d.interno || (d.appartamento && d.appartamento.interno), 40);
  var vani   = v(d.compCamere, 40);
  var acc    = v(d.elemAccessori, 160);
  var pert   = v(d.pertTesto(), 160);
  var arred  = d.ammobiliata === 'si' ? 'ammobiliata' : (d.ammobiliata === 'no' ? 'non ammobiliata' : v('', 80));
  ...
  // Blocco IMU prima casa (solo se porzione e flaggato)
  var imuBlocco = (d.tipoLocazione === 'porzione' && d.primaCase)
    ? ' in caso di locazione parziale di immobile adibito a abitazione principale – prima casa, ai fini delle detrazioni IMU: <i>riservandosene il proprietario una parte, essendo abitazione principale – prima casa – all\'interno della quale mantiene la residenza;</i>'
    : '';

  var html = '';
  html += '<p class="ctr">' + introImm + imuBlocco + '</p>';
  html += '<p>posta in ' + comune + ', Via ' + via + ' n°' + nciv +
    ...
    ', e delle seguenti pertinenze: ' + pert +
...
function generaContratto_A(raw) {
  ...
  var conviventi = v(d.conviventi, 160);
  var statoImm   = v(d.statoImmobile, 200);
  ...
  html += '<p>Il conduttore deve consentire l\'accesso all\'unità immobiliare al locatore, al suo' +
    ...
    ' di recesso anticipato del conduttore o di disdetta del locatore stesso, questi deve' +
    ' consentirne la visita una volta la settimana, per almeno due ore, con esclusione dei giorni' +
    ' festivi (alternativamente indicare altre modalità di accesso).</p>';
...
function generaContratto_B(raw) {
  ...
  html += '<p>Il ' + esigente + ', nel rispetto di quanto previsto dal decreto del Ministro delle' +
    ...
    ' giustifica la transitorietà del contratto in ' + esigenzaTesto + '.</p>';
  ...
  html += '<p>A mente degli articoli 1341 e 1342 del Codice civile, le parti specificamente' +
    ...
    ' stragiudiziale)</i>, 17 <i>(Varie)</i> del presente contratto, 18 (altre clausole).</p>';
  html += '<div class="frow" style="margin-top:8pt;"><div class="fcol"><p>Il locatore</p></div>';
  html += '<div class="fcol"><p>Il conduttore</p></div></div>';
...
function generaContratto_C(raw) {
  ...
  // LOCATORI (Tipo C: formato leggermente diverso — "in qualità di Locatore")
  locs.forEach(function(l, i) {
    ...
    html += '<p>La sig.ra ' + nome + ', nata a ' + luogo + ' il ' + data +
  ...
  // IMMOBILE (Tipo C: struttura leggermente più sintetica)
  ...
  html += '<p>posta in ' + comune + ' via ' + via2 + ', piano ' + piano +
    ', composta di n. ' + vani + ' vani e servizi, ' + arred +
    ' come da separato elenco sottoscritto dalle parti.</p>';
  html += '<p>a) estremi catastali identificativi dell\'unità immobiliare:' +
    ' <b>Foglio ' + foglio + ', p.lla ' + plla + ', sub ' + sub +
    ', categoria ' + cat + ', rendita Euro ' + rendita + '</b>.</p>';
  ...
  html += '<p>c) sicurezza impianti: rispondenti alle normative in materia di sicurezza vigenti' +
    ...
    ' in cui si trovano;</p>';
  html += '<p>d) tabelle millesimali: proprietà ' + tabProp +
    ', riscaldamento autonomo, acqua secondo i consumi.</p>';
  html += '<p>La locazione è regolata dalle pattuizioni seguenti.</p>';
